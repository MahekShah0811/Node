...
module.exports.forgetPassword = async (email) => {
    ...
    // token will be validation only 15 minutes token will be expiry and you can't reset your password

    await user.save();
    const resetLink = `http://localhost:3002/reset-password/${token}`;
    
    // resetLink = frontend page link that show newPassword filed with change password button
...
module.exports.resetPassword = async ({ token, newPassword }) => {
    ...
    if(!user){
        throw new Error("Link Expired!!, Send New Request!!");
    }
